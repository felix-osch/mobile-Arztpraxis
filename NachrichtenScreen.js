    ...
    <View style={styles.container}>
      <ScrollView style={styles.messages}>
        {messages.map((message, index) => (
          <TouchableOpacity key={index} onPress={() => handleOpenMessage('chat')} >
            <View style={styles.message} >
              <MaterialCommunityIcons name="doctor" size={50} color="#007AFF" />
              <View style={styles.messageText}>
                <View style={styles.info}>
                  <Text style={styles.sender}>{message.sender}</Text>
                  <Text style={styles.specialization}>- {message.specialization}</Text>
                </View>
      ...
      <TouchableOpacity style={styles.fab} onPress={() => { handleNewMessage('neueNachricht') }} >
